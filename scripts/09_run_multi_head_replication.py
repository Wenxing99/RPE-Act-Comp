...
def main() -> None:
    ...
    concurrency_level = int(execution_cfg.get("search_workers", 2))
    ...
    if needs_single_best or needs_full:
        for seed in seeds:
            if needs_single_best:
        ...
        print(f"[search] concurrency={concurrency_level} jobs={len(search_jobs)} layer={layer_index} heads={head_indices} seeds={seeds}")
    ...
    manifest = {
        ...
        "device": device_meta,
        "execution": {
            "search_workers": concurrency_level,
        },
        "sanity_checks": {
